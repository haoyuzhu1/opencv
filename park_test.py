...
#from keras.applications.imagenet_utils import preprocess_inputpyth
...
if __name__ == '__main__':
    test_images = [plt.imread(path) for path in glob.glob('test_images/*.jpg')]   #取图片做对比，取最好结果的一个。
    weights_path = 'car1.h5'            #训练好的模型
    video_name = 'parking_video.mp4'    #原视频文件
    class_dictionary = {}               #建立空字典存放分类文件
    class_dictionary[0] = 'empty'       #未占车位存放
    class_dictionary[1] = 'occupied'    #已占车位存放
    park = Parking()                    #封装函数

#下面是执行流程
    park.show_images(test_images)       #显示测试图像
    final_spot_dict = img_process(test_images,park)         #数据预处理
